...
-- Step 2: Update all existing jobs with new UUIDs (only if temp_uuid_id is NULL)
UPDATE "Job" SET "temp_uuid_id" = gen_random_uuid() WHERE "temp_uuid_id" IS NULL;

-- Step 3: Add a temporary text column to job applications to store the new UUID references
ALTER TABLE "JobApplication" ADD COLUMN IF NOT EXISTS "temp_job_uuid" text;

-- Step 4: Update job applications to reference the new UUIDs (only if temp_job_uuid is NULL)
...
-- Step 6: Rename the temporary columns to their final names
DO $$ BEGIN
  BEGIN
