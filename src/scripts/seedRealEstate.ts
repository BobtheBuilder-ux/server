...
import { db } from "../db";
import { users, realEstateCompanies, saleListings, saleListingTypeEnum, saleListingStatusEnum } from "../db/schema";
...
async function seed() {
  ...
  for (const company of companies) {
    ...
    // Check if user exists first to avoid duplicates if re-running
    const existingUsers = await db.select().from(users).where(eq(users.email, company.email));
    ...
    const existingCompanies = await db.select().from(realEstateCompanies).where(eq(realEstateCompanies.email, company.email));
    ...
    // Check if company has listings
    const existingListings = await db.select().from(saleListings).where(eq(saleListings.realEstateCompanyId, companyId));

    if (existingListings.length === 0) {
        for (let i = 0; i < 4; i++) {
            const type = listingTypes[Math.floor(Math.random() * listingTypes.length)] as "Land" | "Property";
            const location = locations[Math.floor(Math.random() * locations.length)];
            
            await db.insert(saleListings).values({
